...
An optional exact FAISS adapter is now implemented behind that benchmark interface for CPU and one NVIDIA GPU. It uses float32 `IndexFlatL2`, records index-build/host-to-device timing, separates GPU upload/search/download when PyTorch tensor interop is available, captures device-memory snapshots, and retains the one-scan reuse/control accounting. Before measurement it must match NumPy candidate sets at every consumed cutoff and row-aligned squared distances, compiled-policy decisions, reranked top-k rows, and retention. Boundary ties are refused because FAISS does not provide the benchmark's stable-row tie contract. Real FAISS 1.10.0 CPU conformance, the initial 10k A100 correctness gate, and the shared-resource memory-reduction gate passed on Slurm job `373268`; no 100k FAISS performance result is claimed yet.
